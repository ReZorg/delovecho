...
DEFAULT_CONFIG = os.path.join(SKILL_DIR, 'templates', 'plan9_cluster_assembly.json')
...
def print_report(result: dict):
    ...
    print(f"\n{'='*60}")
    print(f"  Plan 9 Cluster Topological Analysis: {result['name']}")
    print(f"{'='*60}\n")
...
def main():
    parser = argparse.ArgumentParser(description="Analyze Plan 9 Cluster Topology")
    ...
    # Generate dashboard
    dashboard_path = os.path.join(args.output, "plan9_cluster_topology.png")
    try:
    ...
    # Save analysis JSON
    json_path = os.path.join(args.output, "plan9_cluster_analysis.json")

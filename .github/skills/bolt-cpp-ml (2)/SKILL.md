...
|---|---|---|
| `include/bolt/ai/koboldcpp_provider.hpp` | koboldcpp | Native C++ KoboldCpp provider with dual API mode |
| `src/bolt/ai/koboldcpp_provider.cpp` | koboldcpp | Full implementation (600+ lines) |
| `test/test_koboldcpp_provider.cpp` | cpp-e2e-test-gen | 57 E2E tests across 13 suites |
...
### KoboldCpp C++ Provider (NEW)

The `KoboldCppProvider` class (`include/bolt/ai/koboldcpp_provider.hpp`) provides native C++ integration with KoboldCpp inference servers, implementing the `AICompletionProvider` interface.

**Key features:**
- Dual API mode: KoboldAI native (`/api/v1/generate`) and OpenAI-compatible (`/v1/chat/completions`)
- Auto-detection of server API capabilities
...
- 57 E2E tests in `test/test_koboldcpp_provider.cpp` (13 test suites)
...
config.api_mode = KoboldCppConfig::APIMode::AUTO_DETECT;
KoboldCppProvider provider(config);
provider.initialize();
auto response = provider.chat("Explain C++ templates");
```

### Resolved Issues (2026-03-15)

1. **GGML compile guards**: All 6 GGML-dependent headers now have `BOLT_HAVE_GGML` guards with stub implementations for non-GGML builds.
2. **Hardcoded test paths**: Replaced `/home/ubuntu/bolt-cppml` with `BOLT_TEST_REPO_PATH` compile definition.
3. **Workflow YAML errors**: Fixed heredoc indentation in 3 packaging workflows, rewrote chocolatey workflow, fixed cross-platform matrix references.
4. **Linker errors**: Moved `direct_gguf_inference.cpp` to always-compiled sources (has internal `LLAMA_AVAILABLE` guards).

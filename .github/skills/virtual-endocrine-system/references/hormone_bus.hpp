...
public:
    ...
    void decay_all() noexcept {
        ...
        if (config_.enable_simd) {
            // Process 8 channels at a time (2 iterations for 16 channels)
...
private:
    ...
    void setup_default_channels() {
        ...
        set(HormoneId::RESERVED_1,      10.0f,     1.0f,    0.0f);
        set(HormoneId::RESERVED_2,      10.0f,     1.0f,    0.0f);
    }
    ...
    void recompute_decay_factor(size_t idx) {
        ...
        // Adjusted by global multiplier
        float hl = channel_configs_[idx].half_life * config_.global_decay_multiplier;
        if (hl > 0.0f) {
